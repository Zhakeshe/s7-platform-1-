...
export default function Page() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (found) {
        ...
        // Build local numeric ids for routing, keep remote ids in draft (prefer remoteId if present)
  ...
  const publish = async () => {
    ...
    // Do not auto-inject placeholder lessons; publish empty lessons when none are drafted
    ...
    try {
      ...
      try {
        const payload = {
          ...
          modules: finalModules.map((m, mi) => ({
            // Only send id if remoteId is a valid string; otherwise rely on orderIndex fallback server-side
        ...
        // Persist remote ids back into drafts to keep linkage for future edits
        try {
          const draftRaw = draftKey ? localStorage.getItem(draftKey) : localStorage.getItem("s7_admin_course_draft")
          const draft = draftRaw ? JSON.parse(draftRaw) : null
          if (created?.id && draft && Array.isArray(draft.modules)) {
      ...
      // Update legacy admin storage for the Admin UI cards (prefer server response when available)
      const raw = localStorage.getItem("s7_admin_courses")
      const list = raw ? JSON.parse(raw) : []
