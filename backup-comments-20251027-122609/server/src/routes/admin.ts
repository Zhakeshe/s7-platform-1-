...
router.put("/courses/:courseId", async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Treat any payload module without a known id as a new module to be created (we may shift indices to insert)
    ...
    if (prevModule) {
      ...
      for (let lessonIndex = 0; lessonIndex < (module.lessons || []).length; lessonIndex++) {
        ...
        if (lesson.id && prevLessonsById.has(lesson.id)) {
          ...
          if (sameOrder) {
            // Shift subsequent lessons and create new lesson at target order
            ops.push(
      ...
      if (sameModule) {
        // Insert new module at target order: shift subsequent modules and create new one with nested lessons
        ops.push(
